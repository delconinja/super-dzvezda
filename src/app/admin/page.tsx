...
const GRADES_WITH_CONTENT = [1, 2, 3, 4, 5, 6, 7, 8, 9]
...
export default function AdminPage() {
  ...
  const hasContent = GRADES_WITH_CONTENT.includes(selectedGrade)
  const gradeSubjects = hasContent ? getSubjectsForGrade(selectedGrade) : []
  const gradeContent = hasContent ? getGradeContent(selectedGrade) : {}
        ...
        <div className="max-w-4xl mx-auto flex gap-3">
          {([
            ...
            <button key={s.key} onClick={() => setSection(s.key)}
              ...
              style={{
                background: section === s.key ? '#5C35D4' : 'white',
                color: section === s.key ? 'white' : '#1A1A2E',
                boxShadow: section === s.key ? '0 4px 16px rgba(92,53,212,0.3)' : '0 2px 8px rgba(0,0,0,0.06)',
              }}>
              <div className="font-black text-base">{s.label}</div>
              <div className="text-xs mt-0.5 font-semibold opacity-70">{s.desc}</div>
            </button>
          ))}
        </div>
      </div>
                ...
                <div key={a.id} className="bg-white rounded-3xl p-5"
                  ...
                  <div className="flex flex-wrap items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {a.payout_info && <p className="text-xs mt-1" style={{ color: '#9B9BAA' }}>🏦 {a.payout_info}</p>}
                    </div>
                    <div className="flex flex-col gap-2">
                      {a.status !== 'active' && (
                        <button onClick={() => handleStatus(a.id, 'active')}
                          ...
                          style={{ background: '#6BCB77' }}>✅ Одобри</button>
                      )}
                      {a.status !== 'suspended' && (
                        <button onClick={() => handleStatus(a.id, 'suspended')}
                          ...
                          style={{ background: '#FFE8E8', color: '#C0392B' }}>🚫 Суспендирај</button>
                      )}
                      {a.status === 'active' && (
                        <button onClick={() => setPayoutModal(a)}
                          className="px-4 py-2 rounded-xl text-sm font-black transition-all active:scale-95"
                          style={{ background: '#EDE9FF', color: '#5C35D4' }}>💰 Исплати</button>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {section === 'content' && (<>
          ...
          <div className="flex flex-wrap gap-2">
            {[1,2,3,4,5,6,7,8,9].map((g) => {
              const hasC = GRADES_WITH_CONTENT.includes(g)
              const active = selectedGrade === g
              return (
                <button key={g}
                  onClick={() => { if (hasC) { setSelectedGrade(g); setExpandedSubject(null); setExpandedUnit(null) } }}
                  className="relative px-4 py-2.5 rounded-2xl font-black text-sm transition-all"
                  style={{
                    background: active ? '#5C35D4' : hasC ? 'white' : '#F3F0FF',
                    color: active ? 'white' : hasC ? '#1A1A2E' : '#C4B8EE',
          ...
          {/* Subject cards */}
          {hasContent && gradeSubjects.map((subject) => {
            const units = gradeContent[subject.id] || []
            if (units.length === 0) return null
            const isExpanded = expandedSubject === subject.id
            const totalLessons = units.reduce((s, u) => s + u.lessons.length, 0)
            const totalExercises = units.reduce((s, u) => s + u.lessons.reduce((ls, l) => ls + l.exercises.length, 0), 0)

            return (
              <div key={subject.id} className="bg-white rounded-3xl overflow-hidden"
                style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                {/* Subject header */}
                <button className="w-full px-5 py-4 flex items-center justify-between transition-all"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs font-semibold" style={{ color: '#9B9BAA' }}>
                        {units.length} единици · {totalLessons} лекции · {totalExercises} вежби
                      </p>
                    </div>
                  </div>
                      ...
                      return (
                        <div key={unit.id}>
                          <button className="w-full px-5 py-3 flex items-center justify-between"
                            onClick={() => setExpandedUnit(isUnitExpanded ? null : unitKey)}>
                            <div className="text-left">
                              <p className="font-black text-sm" style={{ color: '#1A1A2E' }}>{unit.title}</p>
                              <p className="text-xs font-semibold" style={{ color: '#9B9BAA' }}>
                                {unit.lessons.length} лекции · {unit.lessons.reduce((s, l) => s + l.exercises.length, 0)} вежби
                              </p>
                            </div>
                            <span className="text-xs font-black ml-3" style={{ color: '#9B9BAA' }}>
                              {isUnitExpanded ? '▲' : '▼'}
                            </span>
                          </button>
                          ...
                          {isUnitExpanded && (
                            <div style={{ background: '#FAFAFA' }}>
                              {unit.lessons.map((lesson, li) => (
                                <div key={lesson.id} className="border-t px-7 py-2.5 flex items-center justify-between"
                                  style={{ borderColor: '#F3F0FF' }}>
                                  <p className="text-sm font-semibold" style={{ color: '#1A1A2E' }}>
                                    {li + 1}. {lesson.title}
                                  </p>
                                  <div className="flex items-center gap-3 ml-3 shrink-0">
                                    <span className="text-xs font-bold" style={{ color: '#9B9BAA' }}>
